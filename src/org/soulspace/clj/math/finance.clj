...
(defn round-financial [precision x]
...
(defn accumulate-by-percentage  [k0 i]
    (* k0 (1 + i)))

(defn discount-by-percentage [kn i]
    (* kn (1 - i)))

(defn cash-value [kn q n]
  (* kn (/ 1 (pow q n))))

(defn accumulated-value [k0 q n]
  (* k0 (pow q n)))

;(defn annuity-accumulated-value)
;(defn annuity-cash-value)

(defn accumulated-rates [r q m]
  (* r 
    (/ (- (pow q m) 1) (- q 1))))

(defn discounted-rates [r q m]
  (* r
    (/ (- (pow q m) 1) (- q 1))
    (/ 1 (pow q m))))

(defn annuity-factor [q n]
  (/ 
    (* (pow q n) (- q 1)) 
    (- (pow q n) 1)))

(defn annuity-rent-Rn [r q n]
  (accumulated-rates r q n))

(defn annuity-rate [k0 q n]
  (* k0 (annuity-factor q n)))

(defn annuity-credit-Km [k0 a q m]
  (- 
    (accumulated-value k0 q m)
    (accumulated-rates a q m)))

(defn annuity-term [k0 a q]
  (/ 
    (log (/ a (- a (* k0 (- q 1)))))
    (log q)))

(defn annuity-rate-by-interest [k0 i it]
  (* k0 (+ i it)))

(defn annuity-term-by-interest [i it]
  (/ 
    (log (+ 1 (/ i it)))
    (log (+ 1 i))))

(defn annuity-redemption-plan [k0 a q]
